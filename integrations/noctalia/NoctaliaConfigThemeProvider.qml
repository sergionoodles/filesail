...
// Reads Noctalia's published theme files for the standalone host. The embedded
// adapter instead uses NoctaliaThemeProvider's live Noctalia singletons.
QtObject {
    id: root
    ...
    readonly property string configDir: {
        ...
        return (xdgConfig.length > 0 ? xdgConfig : String(Quickshell.env("HOME") ?? "") + "/.config") + "/noctalia";
    }
    ...
    function loadColors() {
        try {
            ...
            const next = {
                primary: data.mPrimary, primaryText: data.mOnPrimary,
                surface: data.mSurface, surfaceVariant: data.mSurfaceVariant,
                text: data.mOnSurface, textMuted: data.mOnSurfaceVariant,
                outline: data.mOutline, error: data.mError, errorText: data.mOnError
            };
    ...
    property Timer reloadTimer: Timer {
        interval: 200
        onTriggered: { root.colorsFile.reload(); root.settingsFile.reload(); }
    }

    // Some atomic file replacements do not generate an inotify event for a
    // FileView in every Quickshell build. Polling these two small files keeps
    // the standalone window in sync even when that happens.
    property Timer fallbackReloadTimer: Timer {
        interval: 2500
        repeat: true
        running: true
        onTriggered: { root.colorsFile.reload(); root.settingsFile.reload(); }
