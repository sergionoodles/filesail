...
# Removable-media discovery is additive and degrades cleanly when the test host
# has no system bus or UDisks2 daemon. It must never expose native object paths.
volumes="$(printf '%s\n' '{"id":52,"method":"volumes.list","params":{}}' | "$backend" --serve)"
jq -e '
    .id == 52 and .ok == true and (.available | type == "boolean")
    and .protocolVersion == 1 and (.backendInstance | type == "string")
    and (.revision | type == "string") and (.drives | type == "array")
    and (tostring | contains("/org/freedesktop/UDisks2/") | not)
' <<<"$volumes" >/dev/null

# Control IDs are cryptographically generated by the native helper, and named
# locations resolve through Qt/XDG rather than English directory assumptions.
control_identity="$(printf '%s\n%s\n' \
    '{"id":47,"method":"control.identity","params":{}}' \
    '{"id":48,"method":"control.identity","params":{}}' | "$backend" --serve)"
jq -s -e 'length == 2 and all(.[]; .ok == true and (.value | test("^[A-Za-z0-9_-]{22}$")))
    and .[0].value != .[1].value' <<<"$control_identity" >/dev/null
control_home="$(printf '%s\n' '{"id":49,"method":"control.resolveLocation","params":{"location":"home"}}' | "$backend" --serve)"
jq -e '.id == 49 and .ok == true and (.path | startswith("/"))' <<<"$control_home" >/dev/null
control_relative="$(printf '%s\n' '{"id":50,"method":"control.resolveLocation","params":{"location":"not-a-location"}}' | "$backend" --serve)"
jq -e '.id == 50 and .ok == false and (.error | type == "string")' <<<"$control_relative" >/dev/null
mkdir -- "$test_dir/trailing-space "
control_trailing="$(jq -nc --arg path "$test_dir/trailing-space " \
    '{id: 51, method: "control.resolveLocation", params: {location: $path}}' | "$backend" --serve)"
jq -e --arg path "$test_dir/trailing-space " '.id == 51 and .ok == true and .path == $path' <<<"$control_trailing" >/dev/null
